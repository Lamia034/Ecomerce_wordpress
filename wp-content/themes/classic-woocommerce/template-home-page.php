            ...
            <ul class="p-2 px-3 m-0 categories-list primary-bg-color">
              <?php if(class_exists('woocommerce')){ ?>
                <?php
                  ...
                  $product_categories = get_terms( 'product_cat', $classic_ecommerce_args );
                  $classic_ecommerce_count = count($product_categories);
                  if ( $classic_ecommerce_count > 0 ){
                    foreach ( $product_categories as $classic_ecommerce_product_category ) {
                      $classic_ecommerce_cat_id   = $classic_ecommerce_product_category->term_id;
                      $classic_ecommerce_cat_link = get_category_link( $classic_ecommerce_cat_id );
                      if ($classic_ecommerce_product_category->category_parent == 0) { ?>
                    <li class="d-flex text-white align-items-center"><i class="fa fa-caret-right fs-5 text-white" aria-hidden="true"></i> <a href="<?php echo esc_url(get_term_link( $classic_ecommerce_product_category ) ); ?>">
                    <?php
                  }
                    echo esc_html( $classic_ecommerce_product_category->name ); ?>n</a></li>
                    <?php
                    }
                  }
                ?>
          ...
          <div class="catwrapslider">
            <div class="owl-carousel">
              <?php if( get_theme_mod('classic_ecommerce_slidersection',false) ) { ?>
                <?php $classic_ecommerce_queryvar = new WP_Query(
                    array(
